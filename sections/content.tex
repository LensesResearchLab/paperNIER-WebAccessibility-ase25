...
% TODO It is unclear how truthful these filters are with what impaired users perceive in reality. Hence, further validation is needed.
% TODO There is no alignment or validation of the agent’s simulated behavior against real-world user data, raising concerns about the accuracy and reliability of the agent in reflecting the actual experiences of users with visual impairments.
% ? I thought we had it as a part of this next paragraph
...
% TODO Explain how issues are detected from agent logs — show concrete log features, detection rules/heuristics or classifiers, and example mappings (log → issue).
...
% TODO The paper could be improved by introducing further analysis and discussion on the system overhead, as deploying compute-intensive LLM-based AI agents requires much more resources than conventional static approaches.
% TODO Include a system overhead / compute analysis: runtime, memory, cost, and comparison to conventional static tools; discuss scalability and deployment trade-offs.
% TODO Clarify LMM/LLM choices and config: model(s) used, prompting/fine-tuning strategy, known failure modes, and any preliminary performance observations.
...
% TODO Flesh out the experimental pipeline with concrete steps: site selection, task definitions, agent setup, data collection, metric computation, and analysis methods.
...
% TODO The paper does not specify which accessibility metrics are being tested, nor does it describe how these issues are detected from the agent’s interaction logs.
% ? I thought we had it as a part of this next paragraph
...
% TODO To simulate the interaction of visually impaired users with web interfaces, the authors intend to use perceptual filters. Nevertheless, it is unclear how truthful these filters are with what impaired users perceive in reality. Hence, further validation is needed.
% TODO  Describe how you will align/validate agent behavior against real users: datasets, sample sizes, metrics to compare, and statistical tests.
...
% TODO There is no preliminary evaluation of how well the LMM works in this case. In other words, the proposed pipeline for how the agent will work makes sense; however, how sure are the authors that the agent will work as expected, and that its use will make a difference in this case?
